...
bool ProtobufEncode(std::string &strPb)
{
    ...
    person.set_name("Lzww");      // 设置以set_为前缀
    person.set_age(23);
    ...
    Person::Phone *phone = person.mutable_phone();
    if(!phone)
    {
        std::cout << "mutable_phone failed." << std::endl;
        return false;
    }
    phone->set_number("17781475035");
    phone->set_phone_type(Person::PHONE_HOME);

    // 书籍  repeated数组的处理 序列化的时候
    // add_则是针对repeated的嵌套对象，每次调用都返回一个新的对象，注意和mutable_的区别。
    // 比如Person里面的repeated Book   books   = 5;
    Person::Book *book = person.add_books();
...
static void printPerson(Person::Person &person)
{
    ...
    {
        // 注意引用
        const Person::Phone &phone = person.phone();
    ...
    //序列化的时候：person.add_books()
    // 反序列化：person.books_size() 数组大小
    for (int i = 0; i < person.books_size(); i++)
    {
        const Person::Book &book = person.books(i);
...
/*
ProtobufDecode, size: 119
protobuf encode time testing
protobuf encode 100000 time, need time: 611ms
protobuf decode time testing
protobuf decode 100000 time, need time: 599ms
damn!
*/